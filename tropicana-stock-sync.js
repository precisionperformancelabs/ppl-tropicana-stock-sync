...
async function variants(accessToken) {
  ...
  do {
    ...
    all.push(...d.productVariants.nodes); after=d.productVariants.pageInfo.hasNextPage?d.productVariants.pageInfo.endCursor:null;
  } while(after);
  return all;
}
...
(async()=>{
  ...
  const bySku=new Map();
  for(const v of all){const sku=(v.sku||'').trim(); if(!sku||!feed.has(sku))continue; if(!bySku.has(sku))bySku.set(sku,[]); bySku.get(sku).push(v);}
  let changed=0,unchanged=0,blocked=0;
  for(const [sku,list] of bySku){
    const active=list.filter(v=>v.product.status==='ACTIVE');
    if(active.length!==1){console.error(`BLOCKED sku=${sku} active_matches=${active.length}`);blocked++;continue;}
    ...
    console.log(`VERIFIED sku=${sku} from=${current} to=${wanted}`); changed++;
  }
  console.log(`SYNC_COMPLETE changed=${changed} unchanged=${unchanged} blocked=${blocked} matched_skus=${bySku.size}`);
})().catch(e=>{console.error('SYNC_FAILED',e.stack||e);process.exit(1)});
